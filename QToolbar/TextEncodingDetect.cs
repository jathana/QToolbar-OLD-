...
namespace QToolbar
{
    public class TextEncodingDetect
    {
      ...
      public Encoding DetectEncoding(string file)
      {
         Encoding retval = Encoding.None;
         byte[] buffer;
         ...
         try
         {
            ...
            // Detect encoding
            TextEncodingDetect.Encoding encoding = DetectEncoding(buffer, buffer.Length);
         }
         catch (Exception ex)
         {
            throw new Exception(string.Format("Cannot detect file encoding \"{0}\"", file), ex);
         }
