...
namespace QToolbar
{
    public partial class Frm_FileViewer : Form
    {
         string _Lang = "CSharp (custom highlighter)";
         Language _LangObj;
         string _File = "";
      ...
      private void EnableEditButton(bool enable)
      {
         btnEditFile.Enabled = enable  && !Path.GetExtension(_File).ToLower().Equals(".deploy") 
                                       ...
                                       && !Path.GetExtension(_File).ToLower().Equals(".bat");
      ...
      public void ViewFile(string file, Language lang)
      {
         if (File.Exists(file))
         {
            ...
            {
               fctb.Language = lang;
               fctb.Text = File.ReadAllText(file);
            }
            catch (Exception ex)
            {
      ...
      private void btnOpenFileLocation_ItemClick(object sender, DevExpress.XtraBars.ItemClickEventArgs e)
      {
